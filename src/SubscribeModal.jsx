    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={styles.header}>
          ...
          <p style={styles.subtitle}>Accede al directorio completo de productores agrícolas verificados en campo</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          <div style={styles.fieldWrapper}>
            ...
            <input type="email" placeholder="tu@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required style={styles.input} />
          </div>

          <div style={styles.fieldWrapper}>
            <label style={styles.label}>Empresa (opcional)</label>
            <input type="text" placeholder="Nombre de tu empresa" value={companyName} onChange={(e) => setCompanyName(e.target.value)} style={styles.input} />
          </div>

          <div style={styles.fieldWrapper}>
            <label style={styles.label}>Teléfono (opcional)</label>
            <input type="tel" placeholder="+52 1234567890" value={phone} onChange={(e) => setPhone(e.target.value)} style={styles.input} />
          </div>

          {error && <p style={styles.error}>{error}</p>}

          <button type="submit" disabled={loading || !email} style={{ ...styles.submitBtn, opacity: loading || !email ? 0.6 : 1 }}>
...
const styles = {
  overlay: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, backdropFilter: 'blur(4px)' },
  modal: { backgroundColor: '#fff', borderRadius: '16px', padding: '40px', maxWidth: '440px', width: '90%', position: 'relative', boxShadow: '0 25px 60px rgba(0,0,0,0.3)' },
  closeBtn: { position: 'absolute', top: '16px', right: '16px', background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#666' },
  header: { textAlign: 'center', marginBottom: '20px' },
  badge: { display: 'inline-block', backgroundColor: '#16a34a', color: '#fff', padding: '4px 14px', borderRadius: '20px', fontSize: '12px', fontWeight: '700', letterSpacing: '1px', marginBottom: '12px' },
  ...
  submitBtn: { padding: '14px', borderRadius: '10px', border: 'none', backgroundColor: '#16a34a', color: '#fff', fontSize: '16px', fontWeight: '700', cursor: 'pointer', marginTop: '4px' },
